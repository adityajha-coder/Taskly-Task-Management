class AlarmManager {
    ...
    renderList() {
        try { if (typeof app !== 'undefined' && app && typeof app.renderGamification === 'function') app.renderGamification(); } catch (e) { }
    }
    ...
    updateAlarmForTask(taskId, alarmObj) {
        ...
        if (alarmObj && alarmObj.time) {
            const alarm = {
                ...
                enabled: (new Date(alarmObj.time).getTime() > Date.now()),
                taskId
            };
    ...
    scheduleAll() {
        this.clearTimers();
        this.alarms.forEach(a => { if (a.enabled) this.scheduleAlarm(a); });
    }
